...
在 `.env` 中设置 `PAPERRADAR_EMAIL_ENABLED=1` 和 SMTP 变量后，服务器脚本只会在发现新增论文时发送一封精简中文邮件。脚本会比较更新前后的 digest，只发送新增 arXiv ID 中最新 arXiv 发布日期那一天的论文；邮件发送失败只会写入日志，不会中断常驻调度器。
