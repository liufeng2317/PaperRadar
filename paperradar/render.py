...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .brand-lockup { display: grid; gap: 10px; min-width: 0; }
    h1 { margin: 0; font-size: clamp(2.2rem, 5vw, 4.7rem); line-height: .96; letter-spacing: 0; }
    .source-mark { width: fit-content; display: inline-flex; align-items: center; gap: 8px; border: 1px solid color-mix(in srgb, var(--arxiv) 28%, var(--line)); border-radius: 999px; padding: 6px 10px; color: var(--arxiv); background: var(--arxiv-soft); font-size: .82rem; font-weight: 850; }
    .source-mark::before { content: ""; width: 8px; height: 8px; border-radius: 999px; background: currentColor; }
    html[data-source="eartharxiv"] .source-mark { border-color: color-mix(in srgb, var(--earth) 32%, var(--line)); color: var(--earth); background: var(--earth-soft); }
    ...
    .paper-badges { display: flex; flex-wrap: wrap; gap: 6px; margin: 0 0 8px; }
    .source-badge, .class-badge { display: inline-flex; align-items: center; border-radius: 999px; padding: 2px 8px; font-size: .75rem; font-weight: 800; line-height: 1.55; }
    .source-badge { border: 1px solid color-mix(in srgb, var(--arxiv) 34%, var(--line)); color: var(--arxiv); background: var(--arxiv-soft); }
    .source-badge.eartharxiv { border-color: color-mix(in srgb, var(--earth) 36%, var(--line)); color: var(--earth); background: var(--earth-soft); }
    .class-badge { border: 1px solid var(--soft-line); color: var(--muted); background: var(--panel-soft); font-weight: 700; }
    ...
    .detail-section-title { margin: 0 0 7px; color: var(--muted); font-size: .76rem; font-weight: 850; letter-spacing: .04em; text-transform: uppercase; }
    .summary { display: grid; gap: 8px; margin: 0; }
    .summary ul { margin: 0; padding-left: 1.1rem; }
    .detail-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; margin-top: 0; }
    ...
    @media (max-width: 620px) {
      ...
      .hero { padding: 24px 0 18px; gap: 14px; }
      .topbar { display: grid; gap: 12px; }
      .lang-toggle { justify-self: start; }
      h1 { font-size: clamp(2.05rem, 13vw, 3.2rem); }
      .lede { font-size: 1.04rem; }
      .scope { font-size: .86rem; }
      .dash { gap: 8px; }
      .metric { padding: 10px; }
      main { padding-top: 16px; }
      .toolbar { grid-template-columns: 1fr; }
      .source-tabs { grid-template-columns: 1fr; gap: 7px; margin-bottom: 12px; }
      .source-tabs button { padding: 10px 11px; }
      .panel-title { align-items: start; }
      .paper { padding: 12px; margin-bottom: 9px; }
      .paper-head { display: grid; gap: 8px; }
      .paper h3 { font-size: 1rem; margin-bottom: 7px; }
      .paper-actions { justify-content: start; flex-wrap: wrap; }
      .authors { font-size: .82rem; margin-bottom: 7px; }
      .one-line { font-size: .94rem; margin: 8px 0; }
      .keywords { gap: 5px; margin-top: 8px; }
      .keywords button, .keywords span { font-size: .78rem; padding: 2px 7px; }
      details { margin-top: 10px; padding-top: 9px; }
...
<body>
    ...
    <div class="wrap hero">
      <div class="topbar">
        <div class="brand-lockup">
          ...
          <span class="source-mark" id="source-mark">Loading source</span>
        </div>
  ...
  <script>
    ...
    const classificationLabelZh = (source) => source === "eartharxiv" ? "Subjects" : "Categories";
    ...
    const renderAnalysisDetails = (analysis = {}) => {
      ...
      const body = rows.map(([labelZh, labelEn, valueZh, valueEn]) => {
      ...
      return body ? `<div class="detail-section"><p class="detail-section-title" data-lang="zh">研究要点</p><p class="detail-section-title" data-lang="en">Research details</p><div class="detail-grid">${body}</div></div>` : "";
    };
    const configuredClassifications = (source) => {
      const digest = digests.get(source) || digests.get("arxiv") || digests.get("eartharxiv");
      const config = digest?.config || {};
      return source === "eartharxiv" ? (config.eartharxiv?.subjects || []) : (config.arxiv?.categories || []);
    };
    const prioritizedCategories = (paper = {}) => {
      const source = paper.source || "arxiv";
      const categories = Array.isArray(paper.categories) ? paper.categories.filter(Boolean) : [];
      const configured = configuredClassifications(source).map((item) => String(item).toLowerCase());
      const seen = new Set();
      ...
      const preferred = addUnique(categories.filter((category) => configured.some((needle) => String(category).toLowerCase() === needle)));
      const secondary = addUnique(categories.filter((category) => configured.some((needle) => String(category).toLowerCase().includes(needle) || needle.includes(String(category).toLowerCase()))));
      const rest = addUnique(categories);
      return [...preferred, ...secondary, ...rest];
    };
    const renderClassBadges = (paper = {}) => {
      const source = paper.source || "arxiv";
      const categories = prioritizedCategories(paper);
      const visible = categories.slice(0, 3).map((category) => `<span class="class-badge">${escapeHtml(category)}</span>`).join("");
      const more = categories.length > 3 ? `<span class="class-badge">+${categories.length - 3}</span>` : "";
      return `<div class="paper-badges"><span class="source-badge ${source}">${sourceLabel(source)}</span><span class="class-badge" data-lang="zh">${classificationLabelZh(source)}</span><span class="class-badge" data-lang="en">${classificationLabel(source)}</span>${visible}${more}</div>`;
    };
      ...
      return `
        <article class="paper" data-page-item data-index="${index + 1}">
          <div class="paper-head">
            <div>
              ${renderClassBadges(paper)}
              <h3><a href="${escapeHtml(paper.abs_url || paper.pdf_url || "#")}">${escapeHtml(paper.title || "Untitled")}</a></h3>
              <p class="authors">${escapeHtml(String(paper.published || "").slice(0, 10))} · ${escapeHtml(authorText)}</p>
            </div>
          ...
          <details>
            ...
            <div class="detail-section">
              <p class="detail-section-title" data-lang="zh">摘要要点</p>
              <p class="detail-section-title" data-lang="en">Summary</p>
    ...
    const updateHero = (source) => {
      ...
      const mark = document.getElementById("source-mark");
      if (mark) mark.textContent = source === "eartharxiv" ? "PaperRadar-EarthArXiv" : "PaperRadar-Arxiv";
